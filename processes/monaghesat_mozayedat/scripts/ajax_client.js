...
var trig_uid = "20464240568131f5beaefb4043861496"; // Your trigger UID
hideArrow();
appendAjaxLoading();
$("#record_id").hide();
$("#search_records").hide();
function edit_record(record_id, type, akhz, name, mozayede_shomare, mablagh, dastgahejrai, nahve_sherkat, deadline_asnad, tahvil_bar, deadline_pasokh, alarm, zemanat_nameh, zemanat_nameh_2,insert_date , vahed_marbute, tamin_konnande, category, subcategory, products, haffari_area, file, file_name, file_nahaee, file_name_nahaee, vaze_tamdid, vaze_sherkat) {
  ...
  try {
    $("#subcategory").getControl().prop("disabled", true);
    $("#products").getControl().prop("disabled", true);
    ...
    $("#tamin_konnande").setValue(tamin_konnande || '');
      ...
      // Step 3: Wait briefly to ensure category is set, then set subcategory
      setTimeout(() => {
        $("#subcategory").setValue(subcategory || '');
        $("#subcategory").getControl().prop("disabled", false); // Re-enable
  
        // Step 4: Wait for subcategory to be set, then set products
        setTimeout(() => {
          $("#products").setValue(products || '');
          $("#products").getControl().prop("disabled", false); // Re-enable
  
          // Step 5: Optionally refresh dependent dropdowns manually
          $("#subcategory").refresh(); // Trigger SQL query for subcategory
          $("#products").refresh(); // Trigger SQL query for products
        }, 100); // Small delay to ensure subcategory is set
      }, 100); // Small delay to ensure category is set
...
$(document).ready(function() {
    $("#button_update").find("button").css({
        "background-color": "#007BFF",
        "color": "white"
    });
    $("#create_record").find("button").css({
        "background-color": "#28A745",
        "color": "white"
    });
 	 $("#exit").find("button").css({
...
$("#vahed_marbute").setOnchange(function(newVal, oldVal) {
    ...
    if (newVal == "حفاری" || newVal == "محدوده معدنی") {
        $("#haffari_area").show();
    } else {
        $("#haffari_area").hide();
    }
